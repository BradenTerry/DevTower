// Throwaway capture for the ebook /clear return GIF: a dev with Ebooks on its desk
// (e-reader counter showing) is /cleared, so it stays in its chair, pulls out its
// phone, and a bubble says it returned its Ebooks. Dumps frames to
// screenshots/out/ebookreturn-frames/; assemble the GIF with ffmpeg. Run:
//   npm run screenshots -- -g ebookreturngif
...
const FRAMES = path.join(OUT, "ebookreturn-frames");
...
test("capture: ebookreturngif", async ({ page }) => {
  ...
  // ebook on; seat a dev that already has 3 Ebooks (counter shows on the desk)
  await page.evaluate((s) => {
    window.postMessage({ type: "config", perf: "smooth", books: "ebook" }, "*");
    window.postMessage({ type: "state", agents: [s.agent], rooms: [s.room], boards: { "/repo": s.board } }, "*");
    (window as any).DevTowerCrew.focusIsland("DevTower");
  }, { agent: agent("active", ["code-review", "release", "verify"]), room, board });
  await page.evaluate(() => (document as any).fonts?.ready);
  await page.waitForFunction(() => {
    const t = [...(window as any).DevTowerCrew._instance.toons.values()][0] as any;
    return t && !t.entering && Math.abs(t.targetX - t.x) <= 1 && t.booksShown >= 3;
  }, { timeout: 20000 });
  ...
  await beat(5);                                  // desk e-reader counter "3" showing
  await post(agent("active", [], "clr-1"));       // /clear → seated phone return
  await beat(34);                                 // "Returned my 3 Ebooks" on the phone
  await beat(4);

  console.log(`wrote ${n} frames to ebookreturn-frames/`);
});
