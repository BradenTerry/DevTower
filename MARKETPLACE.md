...
- **Native Source Control + a full file viewer.** The selected worktree's files appear in VS Code's own Source Control panel, split into Staged / Changes with inline stage, unstage, discard, stage-all, unstage-all, **discard-all**, folder-level actions, and commit from the input box, all backed by real `git`. A second *Selected Directory* tree lets you browse and edit **any** file in the worktree, **drag to move** files, and **multi-select + right-click to delete** them - each with a one-time confirm and a "don't ask again" option.
