...
- **Switch directories without leaving the window.** Press a room's **USE DIR** button and DevTower retargets VS Code's own **Explorer** *and* **Source Control** views at that worktree - no second window, no changing your own workspace folder. Hop between repos and worktrees with a click, and the native diffs and each agent's terminal follow the same selected directory.
  - **Explorer.** USE DIR mounts the worktree as a workspace folder, so the built-in Explorer, quick-open (Cmd/Ctrl+P) and search cover **every** file (not just changed ones), editable in place. Move, rename, and delete-to-Trash are the Explorer's native actions; a title-bar toggle shows or hides the project root, and swapping worktrees doesn't reload the window.
  - **Source Control.** A `devtower` SCM provider shows the selected worktree split into Staged / Changes with inline stage, unstage, discard, stage-all, unstage-all, **discard-all**, stash, push / pull / fetch, folder-level actions, and commit from the input box, all backed by real `git`.
- **Pull requests in-scene.** Each room's board shows its worktree's open PR with the number, title, CI checks, and review status, right next to the branch and change counts; a freshly **merged** PR lingers with a purple MERGED badge for a few minutes before it clears. (DevTower won't open a PR for you - just ask the agent in its terminal to create it however you like.)
- **Mini view (compact popout).** A second, canvas-free window beside the tower: a dense table that drills **Projects -> Worktrees -> Agents**, plus **All agents** and **All PRs** tabs. Same live feed as the scene - states, per-branch git stats, and PR/CI status - in a compact list where you can add projects/worktrees, spawn or remove agents, switch the selected directory, and jump to a PR. It's **decoupled from the tower** - open it on its own from the Command Palette, and it keeps updating even when the scene is closed.

## Getting started

1. Install DevTower.
2. Open a folder that has (or will have) Claude Code sessions.
3. The **DevTower** console opens automatically (an editor-area panel, not an activity-bar view). Re-open it any time from the Command Palette -> **DevTower: Open Tower**.
...
| **git** | required | the Source Control view, native diffs, `git worktree add`, and per-room push / pull / fetch |
