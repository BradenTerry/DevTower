...
- **Switch directories without leaving the window.** Press a room's **USE DIR** button and DevTower retargets VS Code's own **Source Control** panel *and* a full **Selected Directory** file viewer at that worktree - no second window, no changing your workspace folder. Hop between repos and worktrees with a click, and the native diffs and each agent's terminal follow the same selected directory.
  - **Source Control** shows the selected worktree split into Staged / Changes with inline stage, unstage, discard, stage-all, unstage-all, **discard-all**, folder-level actions, and commit from the input box, all backed by real `git`.
  - **Selected Directory** lets you browse and edit **any** file in that worktree (not just changed ones) in place, **drag to move** files, and **multi-select + right-click to delete** them - each with a one-time confirm and a "don't ask again" option.
