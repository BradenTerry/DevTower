...
Press **F5** (Run DevTower Extension) to launch an Extension Development Host. The **DevTower** console opens automatically (it is an editor-area panel, not an activity-bar view). Re-open it any time from the Command Palette -> **DevTower: Open Tower**.
...
- **Source Control** (VS Code's own **Source Control** view): DevTower registers a `devtower` SCM provider that mirrors the selected worktree, split into **Staged Changes** and **Changes**, with inline stage (`+`) / unstage (`-`) / discard, stage all / unstage all / **discard all**, stash, per-room push / pull / fetch, commit from the input box, and click-to-diff (native HEAD <-> working tree). In tree mode you can stage, unstage, or discard a whole **folder** at once.
- **File browsing** (VS Code's native **Explorer**): press a room's **USE DIR** button and DevTower mounts that worktree as a workspace folder, so the built-in Explorer, quick-open (Cmd/Ctrl+P) and search all operate on it - every file, not just changed ones, editable in place. Move, rename, and delete-to-Trash are the Explorer's own actions. An Explorer title-bar toggle shows or hides the project root; swapping worktrees does not reload the window.
...
DevTower uses VS Code's **own** Explorer and Source Control views rather than custom panels, so the focused worktree is a place you can actually work - **no second window, no changing your own workspace folder**. Press a room's **USE DIR** button (or switch the selected directory from the mini view) and DevTower retargets the Explorer and the Source Control view at that worktree at once, so hopping between repos and worktrees is one click instead of an Open-Folder / new-window dance. The native diffs and each agent's integrated terminal follow the same selected directory, so the file you browse, the changes you stage, the diff you open, and the terminal you type in all stay pointed at the same place.

- **File browsing (native Explorer).** USE DIR mounts the selected worktree as a workspace folder inside a managed `DevTower.code-workspace`, so the built-in **Explorer**, quick-open (Cmd/Ctrl+P) and ripgrep search all cover the whole worktree - every file, not just changed ones - editable in place without permanently adding it to your workspace. Move, rename, and delete-to-Trash are the Explorer's native actions. Because the worktree is mounted alongside a fixed empty anchor folder, **swapping worktrees does not reload the window**, and a title-bar toggle shows or hides the project root.
- **Source Control (native SCM view).** A `devtower` source-control provider mirrors that **same selected worktree** in VS Code's own Source Control view: staged / unstaged split with inline stage / unstage / discard (and stage-all / unstage-all / discard-all, stash, and folder-level actions in tree mode), per-room push / pull / fetch, commit from the input box, and click-to-diff into the native VS Code diff editor. Switch the selected directory and the SCM view follows - you review and commit one worktree, then another, all in this window.
