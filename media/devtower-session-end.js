...
// matters: the claude process is gone, but DevTower can only INFER that from a
// per-cwd process count on its next poll. When several sessions share a cwd that
// inference is ambiguous — exiting one can cull a still-running sibling's toon
// (or leave the exited one's toon standing), because /exit's final transcript
// write makes the dead session look like the freshest one.
//
// To make exits deterministic we drop an "ended" marker keyed by the exiting
// session's id. Discovery matches it to the dev bound to that exact transcript
// and retires THAT dev immediately, no guessing. We skip the restart reasons
// ("clear" is handled by the SessionStart succession hook, "resume" keeps the
// dev) so only true terminations send the dev home. See src/hooks.ts (reader/
// installer) and src/claude.ts (the retire). We also record the terminal's
// launch id (its --session-id argv, stable across /clear) as a secondary match.
//
...
process.stdin.on("end", () => {
  try {
    ...
    const dir = path.join(os.homedir(), ".claude", "devtower", "ended");
