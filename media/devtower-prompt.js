...
//
// Liveness. Fired the instant you submit a prompt to a session, it drops a
// marker so the dev reads "active" immediately, instead of waiting for the
// transcript mtime to catch up (the model can think for a beat before it writes
// its first line). Reuses the same `active/` marker the resume hook writes,
// which discovery already folds into the session's activity time.
//   ~/.claude/devtower/active/<session_id>.json = { cwd, ts }
//
// (The built-in /rename and /color are consumed by the host before this hook
// runs, so they never reach here; discovery picks them up from history.jsonl.)
//
